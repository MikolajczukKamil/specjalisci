       ...
       public void Add(Order model)
        {
            using (var connection = new NpgsqlConnection("Postgres"))
            {
                ...
                using (var command = new NpgsqlCommand("INSERT INTO orders (id, status, specialist_pricing, specialist_description, start_date, estimated_time, final_price, end_date) VALUES (@id, @status, @specialist_pricing, @specialist_description, @start_date, @estimated_time, @final_price, @end_date)", connection))
                {
